...
Nmsdk-PulseLib реализует импульсные нейронные сети с различными моделями нейронов и механизмами обучения. Библиотека интегрируется с платформой Rdk через систему конфигурационных проектов, позволяя создавать и настраивать модели через XML-файлы без необходимости написания кода.

### Интеграция с Rdk и конфигурационные проекты

Библиотека используется в рамках конфигурационных проектов (экспериментов), которые определяются набором файлов:

- **`project.ini`** — метаданные проекта (имя, шаг моделирования, используемые библиотеки)
- **`Interface.xml`** — настройка GUI и наблюдаемых величин (графики, виджеты)
- **`Model_*.xml`** — структура сети: какие компоненты создаются и как они связаны
- **`Parameters_*.xml`** — численные значения параметров компонентов

```mermaid
flowchart TB
    configs[Configs_Bin] --> engine[NmsdkEngine]
    engine --> pulseLib[PulseLib]
    pulseLib --> nets[NNet_NModel]
    nets --> neurons[Neurons]
    nets --> synapses[Synapses]
    nets --> channels[Channels]
    nets --> classifiers[Classifiers]
    nets --> generators[Generators]
    nets --> trainers[Trainers]
```

Компоненты библиотеки создаются и настраиваются через механизм Rdk Engine, который загружает конфигурационные файлы и создает соответствующие объекты компонентов. Высокоуровневые компоненты `NNet` и `NModel` служат контейнерами для организации нейронов, синапсов и других элементов в единую сеть.
...
### Конфигурационные проекты и эксперименты

Конфигурационные проекты в `Bin/Configs` представляют собой готовые к запуску эксперименты, демонстрирующие различные аспекты работы импульсных нейросетей:

- **Эксперименты с нейронами** (`NM-Neurons/`, `NM-AfferentNeurons/`) — демонстрация различных моделей нейронов и их параметров
- **Эксперименты по обучению** (`STDP-Simple-01/`, `SpikeAnsTrainer/`) — примеры использования механизмов пластичности
- **Классификация** (`SpikeClassifier/`, `SpikeANPA3/`) — примеры классификаторов на основе спайков
- **Когнитивная навигация** (`User/CognitiveNavigation/`) — сложные сети для пространственной навигации
- **Управление движением** (`OldExperiments/MotionControl/`, `OldExperiments/SimplestMotionControl/`) — интеграция с системами управления

Каждый проект может содержать несколько версий модели (`Model_00.xml`, `Model_01.xml`, ...) и соответствующих наборов параметров (`Parameters_00.xml`, `Parameters_01.xml`, ...), что позволяет отслеживать эволюцию эксперимента.

Подробнее о структуре конфигураций см. [Config-Overview.md](Config-Overview.md).
...
`NNet` и `NModel` — высокоуровневые компоненты для организации импульсных нейросетей:

- **`NNet`** — базовый контейнер для нейронов, синапсов и других компонентов сети
- **`NModel`** — расширенная модель с дополнительными возможностями управления и конфигурации

Эти компоненты используются как корневые элементы в конфигурационных проектах (`Model_*.xml`), содержащие всю структуру экспериментальной сети.

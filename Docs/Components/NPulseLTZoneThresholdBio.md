## NPulseLTZoneThresholdBio — компонент PulseLib

**Класс**: `NPulseLTZoneThresholdBio` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseLTZoneThresholdBio", ...)`.  
**Storage**: `ClassName = "NPulseLTZoneThresholdBio"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseLTZoneThresholdBio
```
...
sequenceDiagram
    ...
    participant X as NPulseLTZoneThresholdBio
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseLTZoneThresholdBio]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseLTZoneThresholdBio
Name = NPulseLTZoneThresholdBio1
```

---

## NPulseLTZoneThresholdBio — component PulseLib (EN)

**Class**: `NPulseLTZoneThresholdBio` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseLTZoneThresholdBio", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseLTZoneThresholdBio"` in configs.

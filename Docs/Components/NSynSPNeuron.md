## NSynSPNeuron — компонент PulseLib

**Класс**: `NSynSPNeuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynSPNeuron", ...)`.  
**Storage**: `ClassName = "NSynSPNeuron"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NSynSPNeuron
Name = NSynSPNeuron1
```

---

## NSynSPNeuron — component PulseLib (EN)

**Class**: `NSynSPNeuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynSPNeuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynSPNeuron"` in configs.

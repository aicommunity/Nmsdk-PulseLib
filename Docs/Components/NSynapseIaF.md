## NSynapseIaF — компонент PulseLib

**Класс**: `NSynapseIaF` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynapseIaF", ...)`.  
**Storage**: `ClassName = "NSynapseIaF"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NSynapseIaF
Name = NSynapseIaF1
```

---

## NSynapseIaF — component PulseLib (EN)

**Class**: `NSynapseIaF` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynapseIaF", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynapseIaF"` in configs.

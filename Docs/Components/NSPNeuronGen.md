## NSPNeuronGen — компонент PulseLib

**Класс**: `NSPNeuronGen` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSPNeuronGen", ...)`.  
**Storage**: `ClassName = "NSPNeuronGen"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NSPNeuronGen
Name = NSPNeuronGen1
```

---

## NSPNeuronGen — component PulseLib (EN)

**Class**: `NSPNeuronGen` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSPNeuronGen", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSPNeuronGen"` in configs.

...
**Префикс**: `NS` — **S**ource (источник) или **S**imple (простой).  
...
- `ActionPeriod` — source action period (0 — infinite, > 0 — limited duration)
- `Output` — output signal (generated by derived classes)
- `ActionCounter` — action time counter (internal state)

### Methods

- `SetActionPeriod(value)` — setting action period
- `ADefault()` — setting default parameters
- `AReset()` — state reset (initialize ActionCounter = ActionPeriod)

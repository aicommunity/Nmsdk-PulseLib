## NPulseChannelIzhikevich — компонент PulseLib

**Класс**: `NPulseChannelIzhikevich` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseChannelIzhikevich", ...)`.  
**Storage**: `ClassName = "NPulseChannelIzhikevich"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseChannelIzhikevich
```
...
sequenceDiagram
    ...
    participant X as NPulseChannelIzhikevich
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseChannelIzhikevich]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseChannelIzhikevich
Name = NPulseChannelIzhikevich1
```

---

## NPulseChannelIzhikevich — component PulseLib (EN)

**Class**: `NPulseChannelIzhikevich` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseChannelIzhikevich", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseChannelIzhikevich"` in configs.

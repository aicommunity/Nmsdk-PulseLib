## NSynMotoneuron — компонент PulseLib

**Класс**: `NSynMotoneuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynMotoneuron", ...)`.  
**Storage**: `ClassName = "NSynMotoneuron"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSynMotoneuron
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSynMotoneuron
Name = NSynMotoneuron1
```

---

## NSynMotoneuron — component PulseLib (EN)

**Class**: `NSynMotoneuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynMotoneuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynMotoneuron"` in configs.

## NPulseGeneratorTransit — компонент PulseLib

**Класс**: `NPulseGeneratorTransit` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseGeneratorTransit", ...)`.  
**Storage**: `ClassName = "NPulseGeneratorTransit"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseGeneratorTransit
```
...
sequenceDiagram
    ...
    participant X as NPulseGeneratorTransit
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseGeneratorTransit]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseGeneratorTransit
Name = NPulseGeneratorTransit1
```

---

## NPulseGeneratorTransit — component PulseLib (EN)

**Class**: `NPulseGeneratorTransit` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseGeneratorTransit", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseGeneratorTransit"` in configs.

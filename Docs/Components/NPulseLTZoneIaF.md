## NPulseLTZoneIaF — компонент PulseLib

**Класс**: `NPulseLTZoneIaF` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseLTZoneIaF", ...)`.  
**Storage**: `ClassName = "NPulseLTZoneIaF"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NPulseLTZoneIaF
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NPulseLTZoneIaF
Name = NPulseLTZoneIaF1
```

---

## NPulseLTZoneIaF — component PulseLib (EN)

**Class**: `NPulseLTZoneIaF` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseLTZoneIaF", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseLTZoneIaF"` in configs.

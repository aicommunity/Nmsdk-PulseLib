## NSAfferentNeuron — компонент PulseLib

**Класс**: `NSAfferentNeuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSAfferentNeuron", ...)`.  
**Storage**: `ClassName = "NSAfferentNeuron"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSAfferentNeuron
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NSAfferentNeuron]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NSAfferentNeuron
Name = NSAfferentNeuron1
```

---

## NSAfferentNeuron — component PulseLib (EN)

**Class**: `NSAfferentNeuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSAfferentNeuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSAfferentNeuron"` in configs.

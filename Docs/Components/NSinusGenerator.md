## NSinusGenerator — компонент PulseLib

**Класс**: `NSinusGenerator` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSinusGenerator", ...)`.  
**Storage**: `ClassName = "NSinusGenerator"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSinusGenerator
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSinusGenerator
Name = NSinusGenerator1
```

---

## NSinusGenerator — component PulseLib (EN)

**Class**: `NSinusGenerator` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSinusGenerator", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSinusGenerator"` in configs.

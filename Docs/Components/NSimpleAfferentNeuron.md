## NSimpleAfferentNeuron — компонент PulseLib

**Класс**: `NSimpleAfferentNeuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSimpleAfferentNeuron", ...)`.  
**Storage**: `ClassName = "NSimpleAfferentNeuron"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NSimpleAfferentNeuron
```
...
sequenceDiagram
    ...
    participant X as NSimpleAfferentNeuron
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NSimpleAfferentNeuron]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NSimpleAfferentNeuron
Name = NSimpleAfferentNeuron1
```

---

## NSimpleAfferentNeuron — component PulseLib (EN)

**Class**: `NSimpleAfferentNeuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSimpleAfferentNeuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSimpleAfferentNeuron"` in configs.

## NPulseMembraneIaF — компонент PulseLib

**Класс**: `NPulseMembraneIaF` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseMembraneIaF", ...)`.  
**Storage**: `ClassName = "NPulseMembraneIaF"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NPulseMembraneIaF
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseMembraneIaF]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseMembraneIaF
Name = NPulseMembraneIaF1
```

---

## NPulseMembraneIaF — component PulseLib (EN)

**Class**: `NPulseMembraneIaF` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseMembraneIaF", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseMembraneIaF"` in configs.

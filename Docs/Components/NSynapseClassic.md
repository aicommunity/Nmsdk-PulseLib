## NSynapseClassic — компонент PulseLib

**Класс**: `NSynapseClassic` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynapseClassic", ...)`.  
**Storage**: `ClassName = "NSynapseClassic"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSynapseClassic
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSynapseClassic
Name = NSynapseClassic1
```

---

## NSynapseClassic — component PulseLib (EN)

**Class**: `NSynapseClassic` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynapseClassic", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynapseClassic"` in configs.

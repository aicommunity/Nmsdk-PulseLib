## NSPNeuronBio — компонент PulseLib

**Класс**: `NSPNeuronBio` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSPNeuronBio", ...)`.  
**Storage**: `ClassName = "NSPNeuronBio"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NSPNeuronBio
Name = NSPNeuronBio1
```

---

## NSPNeuronBio — component PulseLib (EN)

**Class**: `NSPNeuronBio` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSPNeuronBio", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSPNeuronBio"` in configs.

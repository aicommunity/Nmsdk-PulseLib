## NPulseSynapseStdp — компонент PulseLib

**Класс**: `NPulseSynapseStdp` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseSynapseStdp", ...)`.  
**Storage**: `ClassName = "NPulseSynapseStdp"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NPulseSynapseStdp
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseSynapseStdp]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseSynapseStdp
Name = NPulseSynapseStdp1
```

---

## NPulseSynapseStdp — component PulseLib (EN)

**Class**: `NPulseSynapseStdp` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseSynapseStdp", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseSynapseStdp"` in configs.

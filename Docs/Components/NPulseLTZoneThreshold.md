## NPulseLTZoneThreshold — компонент PulseLib

**Класс**: `NPulseLTZoneThreshold` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseLTZoneThreshold", ...)`.  
**Storage**: `ClassName = "NPulseLTZoneThreshold"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseLTZoneThreshold
```
...
sequenceDiagram
    ...
    participant X as NPulseLTZoneThreshold
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseLTZoneThreshold]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseLTZoneThreshold
Name = NPulseLTZoneThreshold1
```

---

## NPulseLTZoneThreshold — component PulseLib (EN)

**Class**: `NPulseLTZoneThreshold` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseLTZoneThreshold", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseLTZoneThreshold"` in configs.

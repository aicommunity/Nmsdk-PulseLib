## NPulseNeuronIaF — компонент PulseLib

**Класс**: `NPulseNeuronIaF` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseNeuronIaF", ...)`.  
**Storage**: `ClassName = "NPulseNeuronIaF"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NPulseNeuronIaF
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
Name = NPulseNeuronIaF1
```

---

## NPulseNeuronIaF — component PulseLib (EN)

**Class**: `NPulseNeuronIaF` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseNeuronIaF", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseNeuronIaF"` in configs.

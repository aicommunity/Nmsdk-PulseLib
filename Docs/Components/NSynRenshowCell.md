## NSynRenshowCell — компонент PulseLib

**Класс**: `NSynRenshowCell` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynRenshowCell", ...)`.  
**Storage**: `ClassName = "NSynRenshowCell"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSynRenshowCell
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSynRenshowCell
Name = NSynRenshowCell1
```

---

## NSynRenshowCell — component PulseLib (EN)

**Class**: `NSynRenshowCell` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynRenshowCell", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynRenshowCell"` in configs.

## NSPHebbNeuron — компонент PulseLib

**Класс**: `NSPHebbNeuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSPHebbNeuron", ...)`.  
**Storage**: `ClassName = "NSPHebbNeuron"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSPHebbNeuron
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSPHebbNeuron
Name = NSPHebbNeuron1
```

---

## NSPHebbNeuron — component PulseLib (EN)

**Class**: `NSPHebbNeuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSPHebbNeuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSPHebbNeuron"` in configs.

## NPulsePerseptronIaF — компонент PulseLib

**Класс**: `NPulsePerseptronIaF` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulsePerseptronIaF", ...)`.  
**Storage**: `ClassName = "NPulsePerseptronIaF"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulsePerseptronIaF
```
...
sequenceDiagram
    ...
    participant X as NPulsePerseptronIaF
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulsePerseptronIaF]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulsePerseptronIaF
Name = NPulsePerseptronIaF1
```

---

## NPulsePerseptronIaF — component PulseLib (EN)

**Class**: `NPulsePerseptronIaF` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulsePerseptronIaF", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulsePerseptronIaF"` in configs.

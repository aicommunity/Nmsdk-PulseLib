## NRenshowCell — компонент PulseLib

**Класс**: `NRenshowCell` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NRenshowCell", ...)`.  
**Storage**: `ClassName = "NRenshowCell"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NRenshowCell
Name = NRenshowCell1
```

---

## NRenshowCell — component PulseLib (EN)

**Class**: `NRenshowCell` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NRenshowCell", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NRenshowCell"` in configs.

## NPulseNeuronCable — компонент PulseLib

**Класс**: `NPulseNeuronCable` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseNeuronCable", ...)`.  
**Storage**: `ClassName = "NPulseNeuronCable"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NPulseNeuronCable
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseNeuronCable]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseNeuronCable
Name = NPulseNeuronCable1
```

---

## NPulseNeuronCable — component PulseLib (EN)

**Class**: `NPulseNeuronCable` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseNeuronCable", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseNeuronCable"` in configs.

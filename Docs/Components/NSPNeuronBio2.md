## NSPNeuronBio2 — компонент PulseLib

**Класс**: `NSPNeuronBio2` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSPNeuronBio2", ...)`.  
**Storage**: `ClassName = "NSPNeuronBio2"` в `ClDesc`/`Configs`.
...
sequenceDiagram
    ...
    participant X as NSPNeuronBio2
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```ini
[Component]
ClassName = NSPNeuronBio2
Name = NSPNeuronBio21
```

---

## NSPNeuronBio2 — component PulseLib (EN)

**Class**: `NSPNeuronBio2` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSPNeuronBio2", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSPNeuronBio2"` in configs.

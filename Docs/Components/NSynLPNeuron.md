## NSynLPNeuron — компонент PulseLib

**Класс**: `NSynLPNeuron` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NSynLPNeuron", ...)`.  
**Storage**: `ClassName = "NSynLPNeuron"` в `ClDesc`/`Configs`.
...
```ini
[Component]
ClassName = NSynLPNeuron
Name = NSynLPNeuron1
```

---

## NSynLPNeuron — component PulseLib (EN)

**Class**: `NSynLPNeuron` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NSynLPNeuron", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NSynLPNeuron"` in configs.

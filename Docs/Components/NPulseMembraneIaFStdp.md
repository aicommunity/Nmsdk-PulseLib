## NPulseMembraneIaFStdp — компонент PulseLib

**Класс**: `NPulseMembraneIaFStdp` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseMembraneIaFStdp", ...)`.  
**Storage**: `ClassName = "NPulseMembraneIaFStdp"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseMembraneIaFStdp
```
...
sequenceDiagram
    ...
    participant X as NPulseMembraneIaFStdp
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseMembraneIaFStdp]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseMembraneIaFStdp
Name = NPulseMembraneIaFStdp1
```

---

## NPulseMembraneIaFStdp — component PulseLib (EN)

**Class**: `NPulseMembraneIaFStdp` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseMembraneIaFStdp", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseMembraneIaFStdp"` in configs.

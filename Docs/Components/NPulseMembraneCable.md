## NPulseMembraneCable — компонент PulseLib

**Класс**: `NPulseMembraneCable` — компонент PulseLib (см. реализацию в `NPulseLibrary.cpp`).  
**Регистрация**: `NPulseLibrary.cpp` → `UploadClass("NPulseMembraneCable", ...)`.  
**Storage**: `ClassName = "NPulseMembraneCable"` в `ClDesc`/`Configs`.
...
```mermaid
classDiagram
    UComponent <|-- NPulseMembraneCable
```
...
sequenceDiagram
    ...
    participant X as NPulseMembraneCable
    In-->>X: signals
    X->>X: ACalculate()
    X-->>In: outputs
```
...
```mermaid
flowchart LR
    sig[Signals] --> x[NPulseMembraneCable]
    x --> out[Outputs]
```
...
```ini
[Component]
ClassName = NPulseMembraneCable
Name = NPulseMembraneCable1
```

---

## NPulseMembraneCable — component PulseLib (EN)

**Class**: `NPulseMembraneCable` — PulseLib component (see implementation in `NPulseLibrary.cpp`).

- **Registration**: `UploadClass("NPulseMembraneCable", ...)` in `NPulseLibrary.cpp`.  
- **Storage**: `ClassName = "NPulseMembraneCable"` in configs.
